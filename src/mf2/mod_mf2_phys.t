...
module mod_mf2_phys
  ...
  double precision, public                :: mf2_nu = 1.d-15
  ...
  logical, public, protected              :: mf2_particles = .false.

  !> Whether GLM-MHD is used
  logical, public, protected              :: mf2_glm = .false.
  ...
  double precision, public                :: mf2_glm_alpha = 0.5d0
  ...
  !> The resistivity
  double precision, public                :: mf2_eta = 0.0d0

  !> The hyper-resistivity
  double precision, public                :: mf2_eta_hyper = 0.0d0
  ...
  logical, public, protected :: mf2_divb_4thorder = .false.
  ...
  ! Public methods
  public :: mf2_phys_init
  public :: mf2_to_conserved
  public :: mf2_to_primitive
  public :: mf2_face_to_center
  public :: get_divb
  public :: get_current
  public :: get_normalized_divb
  public :: b_from_vector_potential
  public :: mf2_mag_en_all
  public :: record_force_free_metrics
  {^NOONED
  public :: mf2_clean_divb_multigrid
  }

contains

  !> Read this module's parameters from a file
  subroutine mf2_read_params(files)
    use mod_global_parameters
    use mod_particles, only: particles_eta
    character(len=*), intent(in) :: files(:)
    integer                      :: n

    namelist /mf2_list/ mf2_nu, mf2_vmax, mf2_decay_scale, &
      mf2_eta, mf2_eta_hyper, mf2_glm_alpha, mf2_particles,&
    ...
    do n = 1, size(files)
       ...
       read(unitpar, mf2_list, end=111)
111    close(unitpar)
    end do
 
  end subroutine mf2_read_params

  !> Write this module's parameters to a snapsoht
  subroutine mf2_write_info(fh)
    use mod_global_parameters
  ...
  end subroutine mf2_write_info

  subroutine mf2_phys_init()
    use mod_global_parameters
    use mod_physics
    ...
    call mf2_read_params(par_files)

    physics_type = "mf"
    phys_energy = .false.
    use_particles=mf2_particles
    ...
    case ('multigrid')
       ...
       phys_global_source_after => mf2_clean_divb_multigrid
    }
    case ('glm')
      mf2_glm          = .true.
    ...
    iw_vector(1) = mag(1) - 1   ! 
    ! why like this? see amrvacio/mod_calculate_xw.t where the
    ! vectors are all collected to an array: so that they are transformed
    ! afterwards to cartesian coordinates 
    ...
    if(mf2_glm .and. ndim>1) flux_type(:,psi_)=flux_tvdlf

    phys_get_dt              => mf2_get_dt
    phys_get_cmax            => mf2_get_cmax
    phys_get_cbounds         => mf2_get_cbounds
    phys_get_flux            => mf2_get_flux
    phys_add_source_geom     => mf2_add_source_geom
    phys_add_source          => mf2_add_source
    phys_to_conserved        => mf2_to_conserved
    phys_to_primitive        => mf2_to_primitive
    phys_check_params        => mf2_check_params
    phys_write_info          => mf2_write_info
    ...
    if(mf2_particles) then
      call particles_init()
      phys_req_diagonal = .true.
      ! never allow Hall effects in particles when doing magnetofrictional
      particles_etah=0.0d0
      if(mype==0) then
         write(*,*) '*****Using particles:     with mf2_eta, mf2_eta_hyper :', mf2_eta, mf2_eta_hyper
    ...
    if(stagger_grid) then
      phys_get_ct_velocity => mf2_get_ct_velocity
      phys_update_faces => mf2_update_faces
      phys_face_to_center => mf2_face_to_center
    else if(ndim>1) then
      phys_boundary_adjust => mf2_boundary_adjust
    end if

    {^NOONED
    ! clean initial divb
    if(clean_initial_divb) phys_clean_divb => mf2_clean_divb_multigrid
    }

    ! derive units from basic units
    call mf2_physical_units()

  end subroutine mf2_phys_init
  ...
  subroutine mf2_physical_units()
    ...
    mf2_nu=mf2_nu/unit_time*unit_length**2

    ! get dimensionless maximal mf velocity limit
    mf2_vmax=mf2_vmax/unit_velocity
 
  end subroutine mf2_physical_units

  !> Transform primitive variables into conservative ones
  subroutine mf2_to_conserved(ixI^L,ixO^L,w,x)
    use mod_global_parameters
  ...
  end subroutine mf2_to_conserved

  !> Transform conservative variables into primitive ones
  subroutine mf2_to_primitive(ixI^L,ixO^L,w,x)
    use mod_global_parameters
    integer, intent(in)             :: ixI^L, ixO^L
    double precision, intent(inout) :: w(ixI^S, nw)
    double precision, intent(in)    :: x(ixI^S, 1:ndim)
    ...
    ! nothing to do for mf
  end subroutine mf2_to_primitive



  !> Calculate cmax_idim=csound+abs(v_idim) within ixO^L
  subroutine mf2_get_cmax(w,x,ixI^L,ixO^L,idim,cmax)
    use mod_global_parameters
    ...
    !TODO here and in get_cbounds only one comp of vel is needed but all are computed
    call  frictional_velocity(w,x,ixI^L,ixO^L,vel)
    ! add 1/(dx * mf_nu) as a velocity
    ! TODO should it be a purely diffusive term in get_dt and 
    !cmax(ixO^S)=abs(vel(ixO^S,idim))
    !for the moment we put both
    if(slab_uniform) then
      cmax(ixO^S)= abs(vel(ixO^S,idim)) + 1d0/(mf2_nu * minval(dxlevel(1:ndim)))
    else
  ...
  subroutine mf2_get_cbounds(wLC,wRC,wLp,wRp,x,ixI^L,ixO^L,idim,Hspeed,cmax,cmin)
    use mod_global_parameters
    use mod_variables
    ...
    double precision :: difvel(ixI^S)
    double precision   :: velLC(ixI^S,1:ndir),velRC(ixI^S,1:ndir)

    call  frictional_velocity(wLC,x,ixI^L,ixO^L,velLC)
    call  frictional_velocity(wRC,x,ixI^L,ixO^L,velRC)

    tmp1(ixO^S)=0.5d0*(velLC(ixO^S,idim)+velRC(ixO^S,idim))
    if(slab_uniform) then
      difvel(ixO^S)= 1d0/(mf2_nu * minval(dxlevel(1:ndim)))
    else
      difvel(ixO^S)= 1d0/(mf2_nu * minval(block%ds(ixO^S,1:ndim),dim=ndim+1))
    end if
    if(present(cmin)) then
      cmax(ixO^S,1)=max(tmp1(ixO^S)+difvel(ixO^S),zero)
      cmin(ixO^S,1)=min(tmp1(ixO^S)-difvel(ixO^S),zero)
    else
      cmax(ixO^S,1)=abs(tmp1(ixO^S))+difvel(ixO^S)
    end if

  end subroutine mf2_get_cbounds

  !> prepare velocities for ct methods
  subroutine mf2_get_ct_velocity(vcts,wLp,wRp,ixI^L,ixO^L,idim,cmax,cmin)
    use mod_global_parameters
    ...
    integer                         :: idimE,idimN
    double precision   :: velLp(ixI^S,1:ndir),velRp(ixI^S,1:ndir)

    call  frictional_velocity(wLp,block%x(ixI^S,1:ndim),ixI^L,ixO^L,velLp)
    call  frictional_velocity(wRp,block%x(ixI^S,1:ndim),ixI^L,ixO^L,velRp)
    ...
    case('uct_contact')
      ...
      vcts%vnorm(ixO^S,idim)=0.5d0*(velLp(ixO^S,idim)+velRp(ixO^S,idim))
    case('uct_hll')
      ...
      ! Store velocities
      vcts%vbarLC(ixO^S,idim,1)=velLp(ixO^S,idimN)
      vcts%vbarRC(ixO^S,idim,1)=velRp(ixO^S,idimN)
      ...
      vcts%vbarLC(ixO^S,idim,2)=velLp(ixO^S,idimE)
      vcts%vbarRC(ixO^S,idim,2)=velRp(ixO^S,idimE)
  ...
  end subroutine mf2_get_ct_velocity


  !> Calculate fluxes within ixO^L.
  subroutine mf2_get_flux(wC,w,x,ixI^L,ixO^L,idim,f)
    use mod_global_parameters
    use mod_usr_methods
    use mod_geometry
    ...
    do idir=1,ndir
      if (idim==idir) then
        ...
        f(ixO^S,mag(idir))=vel(ixO^S,idim)*B(ixO^S,idir)-B(ixO^S,idim)*vel(ixO^S,idir)
      end if
    end do
    if (mf2_glm) then
  ...
  subroutine mf2_add_source(qdt,dtfactor,ixI^L,ixO^L,wCT,wCTprim,w,x,qsourcesplit,active)
    ...
    if (.not. qsourcesplit) then
      ...
      if (abs(mf2_eta)>smalldouble)then
        active = .true.
        call add_source_res2(qdt,ixI^L,ixO^L,wCT,w,x)
      end if

      if (mf2_eta_hyper>0.d0)then
        active = .true.
  ...
  end subroutine mf2_add_source

  subroutine frictional_velocity(w,x,ixI^L,ixO^L,vel)
    use mod_global_parameters

    integer, intent(in) :: ixI^L, ixO^L
    double precision, intent(in) :: x(ixI^S,1:ndim)
    double precision, intent(in) :: w(ixI^S,1:nw)
    double precision, intent(out) :: vel(ixI^S,1:ndir)

    double precision :: xmin(ndim),xmax(ndim)
    double precision :: decay(ixO^S)
    double precision :: current(ixI^S,7-2*ndir:3),tmp(ixO^S)
    double precision :: B(ixI^S,1:ndir)
    ...
    !NO MORE EXTRAPOLATION with B0 fiels
!{
...
!\}
    vel(ixO^S,:)=0.d0
    ! calculate Lorentz force, faster than using cross_product?
    do idir=1,ndir; do jdir=idirmin,3; do kdir=1,ndir
      if(lvc(idir,jdir,kdir)/=0) then
        if(lvc(idir,jdir,kdir)==1) then
          vel(ixO^S,idir)=vel(ixO^S,idir)+current(ixO^S,jdir)*B(ixO^S,kdir)
        else
          vel(ixO^S,idir)=vel(ixO^S,idir)-current(ixO^S,jdir)*B(ixO^S,kdir)
        end if
      end if
...
!    endwhere
    ...
    decay(ixO^S)=1.d0
    do idir=1,ndim
      if(mf2_decay_scale(2*idir-1)>0.d0) decay(ixO^S)=decay(ixO^S)*&
         (1.d0-exp(-(x(ixO^S,idir)-xmin(idir))/mf2_decay_scale(2*idir-1)))
      if(mf2_decay_scale(2*idir)>0.d0) decay(ixO^S)=decay(ixO^S)*&
         (1.d0-exp((x(ixO^S,idir)-xmax(idir))/mf2_decay_scale(2*idir)))
    end do

    ! saturate mf velocity at mf2_vmax
    tmp(ixO^S)=sqrt(sum(vel(ixO^S,1:ndir)**2,dim=ndim+1))/mf2_vmax+1.d-12
    tmp(ixO^S)=dtanh(tmp(ixO^S))/tmp(ixO^S)
    do idir=1,ndir
      vel(ixO^S,idir)=vel(ixO^S,idir)*tmp(ixO^S)*decay(ixO^S)
    end do
  ...
  subroutine add_source_hyperres(qdt,ixI^L,ixO^L,wCT,w,x)
    ...
    ehyper(ixA^S,1:ndir) = - tmpvec(ixA^S,1:ndir)*mf2_eta_hyper
  ...
  subroutine add_source_glm(qdt,ixI^L,ixO^L,wCT,w,x)
    ...
    call get_divb(wCT,ixI^L,ixO^L,divb, mf2_divb_4thorder)

    ! dPsi/dt =  - Ch^2/Cp^2 Psi
    if (mf2_glm_alpha < zero) then
      w(ixO^S,psi_) = abs(mf2_glm_alpha)*wCT(ixO^S,psi_)
    else
      ! implicit update of Psi variable
      ! equation (27) in Mignone 2010 J. Com. Phys. 229, 2117
      if(slab_uniform) then
        w(ixO^S,psi_) = dexp(-qdt*cmax_global*mf2_glm_alpha/minval(dxlevel(:)))*w(ixO^S,psi_)
      else
        w(ixO^S,psi_) = dexp(-qdt*cmax_global*mf2_glm_alpha/minval(block%ds(ixO^S,:),dim=ndim+1))*w(ixO^S,psi_)
      end if
    end if
  ...
  subroutine add_source_powel(qdt,ixI^L,ixO^L,wCT,w,x)
    ...
    ! calculate velocity
    call  frictional_velocity(wCT,x,ixI^L,ixO^L,v)
  ...
  subroutine add_source_linde(qdt,ixI^L,ixO^L,wCT,w,x)
    ...
    ! Calculate div B
    ixp^L=ixO^L^LADD1;
    call get_divb(wCT,ixI^L,ixp^L,divb, mf2_divb_4thorder)
  ...
  !TODO for fourthorder add + 1 nghostcells?
  ...
  subroutine mf2_get_dt(w,ixI^L,ixO^L,dtnew,dx^D,x)
    use mod_global_parameters
    use mod_usr_methods
    ...
    ^D&dxarr(^D)=dx^D;
    if (mf2_eta>zero)then
       dtnew=dtdiffpar*minval(dxarr(1:ndim))**2/mf2_eta
    else if (mf2_eta<zero)then
       call get_current(w,ixI^L,ixO^L,idirmin,current)
    ...
    if(mf2_eta_hyper>zero) then
      if(slab_uniform) then
        dtnew=min(dtdiffpar*minval(dxarr(1:ndim))**4/mf2_eta_hyper,dtnew)
      else
        dtnew=min(dtdiffpar*minval(block%ds(ixO^S,1:ndim))**4/mf2_eta_hyper,dtnew)
      end if
    end if
  ...
  subroutine mf2_add_source_geom(qdt,dtfactor,ixI^L,ixO^L,wCT,w,x)
    ...
    case (spherical)
       ...
       ! b2
       if(.not.stagger_grid) then
         tmp(ixO^S)=vel(ixO^S,1)*wCT(ixO^S,mag(2)) &
              -vel(ixO^S,2)*wCT(ixO^S,mag(1))
         if(B0field) then
           tmp(ixO^S)=tmp(ixO^S)+(vel(ixO^S,1)*block%B0(ixO^S,2,0) &
                -vel(ixO^S,2)*block%B0(ixO^S,1,0))
         end if
         if(mf2_glm) then
           tmp(ixO^S)=tmp(ixO^S) &
       ...
       if(ndir==3) then
         ! b3
         if(.not.stagger_grid) then
           tmp(ixO^S)=(vel(ixO^S,1)*wCT(ixO^S,mag(3)) &
                -vel(ixO^S,3)*wCT(ixO^S,mag(1))) {^NOONED &
                -(vel(ixO^S,3)*wCT(ixO^S,mag(2)) &
                -vel(ixO^S,2)*wCT(ixO^S,mag(3)))*dcos(x(ixO^S,2)) &
                /dsin(x(ixO^S,2)) }
           if (B0field) then
              tmp(ixO^S)=tmp(ixO^S)+(vel(ixO^S,1)*block%B0(ixO^S,3,0) &
                   -vel(ixO^S,3)*block%B0(ixO^S,1,0)){^NOONED &
                   -(vel(ixO^S,3)*block%B0(ixO^S,2,0) &
  ...
  end subroutine mf2_add_source_geom

  !> Compute 2 times total magnetic energy
  function mf2_mag_en_all(w, ixI^L, ixO^L) result(mge)
    use mod_global_parameters
    integer, intent(in)           :: ixI^L, ixO^L
    double precision, intent(in)  :: w(ixI^S, nw)
  ...
  end function mf2_mag_en_all



  subroutine mf2_modify_wLR(ixI^L,ixO^L,qt,wLC,wRC,wLp,wRp,s,idir)
    use mod_global_parameters
    use mod_usr_methods
  ...
  end subroutine mf2_modify_wLR

  subroutine mf2_boundary_adjust(igrid,psb)
    use mod_global_parameters
  ...
  end subroutine mf2_boundary_adjust
  ...
  {^NOONED
  subroutine mf2_clean_divb_multigrid(qdt, qt, active)
    use mod_forest
    use mod_global_parameters
    use mod_multigrid_coupling
    use mod_geometry
    ...
    do n = 1, 2*ndim
       ...
       case (bc_periodic)
          ! Nothing to do here
       case default
          write(*,*) "mf2_clean_divb_multigrid warning: unknown boundary type"
          mg%bc(n, mg_iphi)%bc_type = mg_bc_dirichlet
          mg%bc(n, mg_iphi)%bc_value = 0.0_dp
       end select
    end do
    ...
    do iigrid = 1, igridstail
       ...
       if(stagger_grid) then
         ...
         call mf2_face_to_center(ixM^LL,ps(igrid))
       else
         do idim = 1, ndim
  ...
  end subroutine mf2_clean_divb_multigrid
  }

  subroutine mf2_update_faces(ixI^L,ixO^L,qt,qdt,wprim,fC,fE,sCT,s,vcts)
    use mod_global_parameters
  ...
  end subroutine mf2_update_faces
  ...
  subroutine update_faces_average(ixI^L,ixO^L,qt,qdt,fC,fE,sCT,s)
    ...
    if(mf2_eta/=zero) call get_resistive_electric_field(ixI^L,ixO^L,sCT,s,jce)
      ...
      do idim2=1,ndim
        ...
        do idir=7-2*ndim,3! Direction of line integral
          ...
          if (lvc(idim1,idim2,idir)==1) then
            ...
            if(mf2_eta/=zero) fE(ixC^S,idir)=fE(ixC^S,idir)+jce(ixC^S,idir)
  ...
  subroutine update_faces_contact(ixI^L,ixO^L,qt,qdt,wp,fC,fE,sCT,s,vcts)
    ...
    do idim1=1,ndim; do idim2=1,ndim; do idir=7-2*ndim,3
      if(lvc(idim1,idim2,idir)==1)then
         ECC(ixI^S,idir)=ECC(ixI^S,idir)+wp(ixI^S,mag(idim1))*vel(ixI^S,idim2)
  ...
  subroutine mf2_face_to_center(ixO^L,s)
    use mod_global_parameters
  ...
  end subroutine mf2_face_to_center
  ...
  subroutine record_force_free_metrics()
    ...
    if(mype==0) then
      ...
      if(.not.logmfopened) then
        ! generate filename
        write(filename,"(a,a)") TRIM(base_filename), "_mf2_metrics.csv"
